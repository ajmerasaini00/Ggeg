...
MAX_SIZE_BYTES = 50 * 1024 * 1024  # 50 MB per PDF

@bot.on_message(filters.document & filters.private)
async def handle_txt_file(client: Client, m: Message):
    # सिर्फ txt files ही accept करो
    if not m.document.file_name.endswith(".txt"):
        await m.reply_text("❌ कृपया सिर्फ .txt फ़ाइल भेजें जिसमें PDF links हों।")
        return

    sent = await m.reply_text("📥 TXT फ़ाइल डाउनलोड हो रही है...")

    # step 1: txt file Telegram से डाउनलोड करो
    txt_path = await m.download()

    # step 2: txt file से सारे links पढ़ लो
    with open(txt_path, "r", encoding="utf-8") as f:
        links = [line.strip() for line in f if line.strip()]

    if not links:
        await sent.edit_text("❌ TXT फ़ाइल में कोई valid लिंक नहीं मिला।")
        return

    await sent.edit_text(f"✅ {len(links)} लिंक मिले। अब PDFs डाउनलोड हो रही हैं...")

    # step 3: हर लिंक से PDF डाउनलोड करके भेजो
    for idx, pdf_url in enumerate(links, start=1):
        tmp_path = None
        try:
            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                async with session.get(pdf_url) as resp:
                    if resp.status != 200:
                        await m.reply_text(f"❌ Link {idx} फेल: HTTP {resp.status}")
                        continue

                    content_length = resp.headers.get("Content-Length")
                    if content_length and int(content_length) > MAX_SIZE_BYTES:
                        await m.reply_text(f"❌ Link {idx} बहुत बड़ी फ़ाइल है (limit {MAX_SIZE_BYTES} bytes)")
                        continue
            ...
            # send PDF to user
            await m.reply_document(document=tmp_path, file_name=f"document_{idx}.pdf")

        except asyncio.TimeoutError:
            await m.reply_text(f"❌ Link {idx} timeout हो गया।")
        except Exception as e:
            await m.reply_text(f"❌ Link {idx} Error: {e}")
        finally:
            if tmp_path and os.path.exists(tmp_path):
                os.remove(tmp_path)

    await sent.edit_text("✅ सारे links process हो गए।")
    os.remove(txt_path)
# .....,.....,.......,...,.......,....., .....,.....,.......,...,.......,.....,
